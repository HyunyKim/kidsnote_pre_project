...
protocol BookCollectionViewLayout {
    func createEBookListLayout() -> UICollectionViewLayout
...
extension BookCollectionViewLayout {
    ...
    func createShelfListLayout() -> UICollectionViewLayout {
        let layout = UICollectionViewCompositionalLayout { (sectionIndex: Int,
                                                            layoutEnvironment: NSCollectionLayoutEnvironment) -> NSCollectionLayoutSection? in
    ...
    func emitDataMysheifVoloumSource(items: [EBook], hasMore: Bool) -> [SearchResultSectionModel] {
        let ebookItemSection = SearchResultSectionModel.eBookItemSection(
            items: items.isEmpty ? [SearchResultSectionItem.emptyView] : items.map { SearchResultSectionItem.eBookItem(item: $0) } )
        let loadMoreSection = SearchResultSectionModel.loadMoreSection(item: hasMore ? [SearchResultSectionItem.loadMore] : [])
        return [ebookItemSection,loadMoreSection]
    }
}
